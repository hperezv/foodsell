	      ...
	      for($i = 1;$i <= $rows; $i++)
	      {
	        ...
	        for($z = 0; $z < count($fields_table); $z++)
	        {
	        	if($letras[$z]=='E'){
	        		...
	        		$fechaexcel=$objPHPExcel->getActiveSheet()->getCell($letras[$z].$i)->getCalculatedValue();
	        		//$fechexphp=new Datetime("2010-08-27");
	        		//echo (string)$fechexphp;
	        		
	        		//echo date_format((int)$fechaexcel, 'Y-m-d H:i:s');
	        		//$date = date_create($fechaexcel);
	        		//echo date($date, 'Y-m-d H:i:s');
	        		//echo date('Y-m-d', ($fechaexcel -35011)*24*3600);
 
	        		$fechanew=substr($fechaexcel,6,4).'-'.substr($fechaexcel,3,2).'-'.substr($fechaexcel,0,2);
	        		//echo $fechaexcel;

	        	   $sql[$i][trim($fields_table[$z])] = date('Y-m-d', ($fechaexcel -25569)*24*3600);
